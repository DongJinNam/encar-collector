...
def test_autohub():
    car_count = encar_crawler.get_car_count(constants.API_ENCAR_URL + constants.SEARCH_CAR_LIST_URL_PREMIUM,
                                                 constants.AUTOHUB_COOLSEAT_FILTER)
    assert car_count > 0
    print("오토허브(통풍시트) 전체 : " + str(car_count))
    ...
    print("오토허브(통풍시트) 1인신조 : " + str(len(unused_car_list)))

    df = pd.DataFrame(unused_car_list)
    df.to_excel('오토허브(통풍시트)_1인신조_' + str(time.strftime("%y%m%d")) + '.xlsx')